...
export default class S3Controller {
  ...
  async download(req: Request, res: Response, next: NextFunction) {
    ...
    async listObjectsEncrypted(req: Request, res: Response, next: NextFunction) {
        try {
            const query = listObjectsQuerySchema.parse(req.query);
    ...
    async uploadEncrypted(req: Request, res: Response, next: NextFunction) {
        try {
            const file = uploadSchema.parse(req.file);
            const caminho = downloadSchema.parse(req.body).caminho;
            ...
            const s3Service = new S3Service(new S3Repository(s3));
            await s3Service.uploadEncrypted(file as unknown as Express.Multer.File, caminho);
    ...
    async downloadEncrypted(req: Request, res: Response, next: NextFunction) {
        try {
            const raw = req.query.fileName;
            if (raw === undefined || raw === "" || Array.isArray(raw)) {
                return res.status(400).json({
            ...
            const s3Service = new S3Service(new S3Repository(s3));
            const file = await s3Service.downloadEncrypted(fileName);
